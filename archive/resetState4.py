...
def create_model(inputs):
    ...
    z = keras.layers.GRU(128, return_sequences=True, dropout=0.3, recurrent_regularizer='l2', stateful= False) (inputs, training=True)
    z3 = keras.layers.GRU(128,return_sequences=True, dropout=0.3, recurrent_regularizer='l2', stateful= False) (z, training=True)
    z4 = keras.layers.GRU(128,return_sequences=True, dropout=0.3, recurrent_regularizer='l2', stateful= False) (z3, training=True)
    z2 = keras.layers.GRU(128, dropout=0.3, recurrent_regularizer='l2', stateful = False) (z4, training=True)
  ...
  """
    ...
    def on_batch_begin(self, batch, logs=None ):
        ...
        if self.customer in states_dict:
            ...
            # self.state1 = states_dict1[self.customer]
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        # states_dict1[self.customer] = self.state1
    ...
    def on_test_batch_begin(self, batch, logs=None):
        ...
        if self.customer in states_dict:
            ...
            # self.state1 = states_dict1[self.customer]
            # gru_layer1.reset_states(states = self.state1[0].numpy())
            # self.state2 = states_dict2[self.customer]
            # gru_layer2.reset_states(states = self.state2[0].numpy())
            # self.state3 = states_dict3[self.customer]
            # gru_layer3.reset_states(states = self.state3[0].numpy())
        else:
            #reset state and store it
            gru_layer.reset_states()
...
checkpoint_filepath = '/tmp/StateperCard1F_v1.h5'
...
csvLog = tf.keras.callbacks.CSVLogger("training_log_4F_ResetState_v1.csv", 
                             separator=',', 
                             append=True)
...
history = model1.fit(
    ...
    callbacks = [es_callback, reduce_lr, cb, model_checkpoint_callback, csvLog]#, StatePerCard([customersPerBatch, customersPerBatch_Val, gru_layer])],#, gru_layer1, gru_layer2, gru_layer3]) ],
    #verbose = 2
    #shuffle = True
)
...
plt.savefig("ResetState4_v1.png")
...
test_scores = model1.evaluate(dataset_test, callbacks=[csvLog])
...
test_predictions_baseline = model1.predict(test_features)
