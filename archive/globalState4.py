...
csvLog = tf.keras.callbacks.CSVLogger("training_log_4F_GlobalState_v2.csv", 
                             separator=',', 
                             append=True)
...
plt.savefig("GlobalState4_v2.png")
...
test_scores = model1.evaluate(dataset_test)#, callbacks=StatePerCard([customersPerBatch, customersPerBatch_test, gru_layer]))#, gru_layer1, gru_layer2, gru_layer3]))
...
test_predictions_baseline = model1.predict(test_features)#, callbacks=StatePerCard([customersPerBatch, customersPerBatch_test, gru_layer, gru_layer1, gru_layer2, gru_layer3]))
