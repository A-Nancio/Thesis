...
frames = [result, validation, test]
len1 = result.shape[0]
len2 = validation.shape[0]
len3 = test.shape[0]
Data = pd.concat(frames)
Data['amount_bins'] = pd.qcut(Data['amount_zscore'], q=4, labels = False, precision=0)

result = Data.iloc[:len1,:]
validation = Data.iloc[len1:(len1+len2),:]
test = Data.iloc[len1+len2:,:]
...
for pack in dataset_train:
    inputs, targets = pack
    customer = inputs[0][0][-2].numpy()
    #age = inputs[0][0][-3].numpy()
    gender = inputs[0][0][-4].numpy()
    merchant = inputs[0][0][-6].numpy()
    category = inputs[0][0][-7].numpy()
    amount_bins  = inputs[0][0][-1].numpy()
    customersPerBatch.append(customer)
    sharedPerBatch.append(amount_bins)
    i = i +1
...
for pack in dataset_Val:
    ...
    sharedPerBatch_Val.append(amount_bins)
    i = i +1
...
def create_model(inputs):
    ...
    z = keras.layers.GRU(128, dropout=0.3, recurrent_regularizer='l2', stateful=True) (inputs, training=True)
    z2 = keras.layers.GRU(128, dropout=0.3, recurrent_regularizer='l2', stateful = True) (inputs, training=True)
    # pp = tf.keras.layers.GRU(128, stateful = True) (z)
    #z = gru_layer(inputs)
    concat = tf.keras.layers.Concatenate()([z, z2])
    ...
    output3 = tf.keras.layers.Dense(24)(concat)
...
class StatePerCard(keras.callbacks.Callback):
  ...
  """
    ...
    def __init__(self, data_layer):
        ...
        self.shared_data = data_layer[2]
        self.data_val = data_layer[1]
        self.shared_data_val = data_layer[3]
        self.state = None
        self.state1 = None
        #self.state2 = None
        #self.state3 = None
    ...
    def on_batch_begin(self, batch, logs=None ):
        ...
        if self.shared in states_dict_shared:
            self.state1 = copy.deepcopy(states_dict_shared[self.shared])
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        states_dict_shared[self.customer] = copy.deepcopy(gru_layer1.states)
...
checkpoint_filepath = '/tmp/SharedAmount4.h5'
...
csvLog = tf.keras.callbacks.CSVLogger("training_log_4F_SharedAmount4.csv", 
                             separator=',', 
                             append=True)
...
history = model1.fit(
    ...
    callbacks = [es_callback, reduce_lr, cb, model_checkpoint_callback, csvLog, StatePerCard([customersPerBatch, customersPerBatch_Val, sharedPerBatch, sharedPerBatch_Val]) ],
    #verbose = 2
    #shuffle = True
)
...
plt.savefig("SharedStateAmount_v1.png")
...
for pack in dataset_test:
    ...
    gender = inputs[0][0][-4].numpy()
    merchant = inputs[0][0][-6].numpy()
    category = inputs[0][0][-8].numpy()
    amount_bins = inputs[0][0][-1].numpy()
    customersPerBatch_test.append(customer)
    sharedPerBatch_test.append(amount_bins)
    i = i +1

#train_predict = model1.predict(X_train)
#test_predict = model1.predict(dataset_train[0])
model1.load_weights(checkpoint_filepath)
test_scores = model1.evaluate(dataset_test, callbacks=StatePerCard([customersPerBatch, customersPerBatch_test, sharedPerBatch, sharedPerBatch_test]))
...
test_predictions_baseline = model1.predict(test_features, callbacks=StatePerCard([customersPerBatch, customersPerBatch_test, sharedPerBatch, sharedPerBatch_test]))
