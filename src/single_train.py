...
training_set = tf.data.Dataset.from_tensor_slices(
    (np.load(f'src/data/train/fold_4/train_seq_ids.npy').astype(int), 
     np.load(f'src/data/train/fold_4/train_seq_labels.npy').astype(float))
     ).batch(BATCH_SIZE)

validation_set = tf.data.Dataset.from_tensor_slices(
    (np.load(f'src/data/train/fold_4/val_seq_ids.npy').astype(int), 
     np.load(f'src/data/train/fold_4/val_seq_labels.npy').astype(float))
     ).batch(BATCH_SIZE)
    ...
    with open(f'src/training_runs/train_{production_model.name}.csv', 'w') as train_f:
        with open(f'src/training_runs/validation_{production_model.name}.csv', 'w')  as val_f:
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                print(f"{train_model.name} validation: [epoch {epoch}], results: {validation_results}")
                train_model.reset_metrics()
                if "double" in train_model.name:
                    train_model.reset_gru()
                
                train_writer.writerow([epoch] + train_results)
                val_writer.writerow([epoch] + validation_results)
